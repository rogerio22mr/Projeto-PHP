...
class CafeApi extends Controller
{
     ...
     * @return CafeApi
     */
    protected function back(array $reponse = null): CafeApi
    {
        if (!empty($reponse)) {
            $this->reponse = (!empty($this->reponse) ? array_merge($this->reponse, $reponse) : $reponse);
        }

        echo json_encode($this->reponse, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
        return $this;
    }
    ...
    protected function requestLimit(string $endpoint, int $limit, int $seconds, bool $attempt = false): bool
    {
        ...
        if (empty($cache[$endpoint]) || $cache[$endpoint]->limit <= time()) {
            if (!$attempt) {
                $cache[$endpoint] = [
                    "limit" => $limit,
                    "requests" => 1,
        ...
        if ($cache[$endpoint]->requests >= $limit) {
            $this->call(
                400,
                "request_limit",
                "Você excedeu o limit de requisições para essa ação"
            )->back();
