...
<div class="mce_upload" style="z-index: 997">
    <div class="mce_upload_box">
        <form class="app_form"
            action="<?= url("/admin/blog/post"); ?>"
            method="post" enctype="multipart/form-data">
            <input type="hidden" name="upload" value="true" />
            <label>
                <label class="legend">Selecione uma imagem JPG ou PNG:</label>
                <input accept="image/*" type="file" multiple name="image" required />
            </label>
...
<section class="dash_content_app">
    ...
    <?php else: ?>
    <header class="dash_content_app_header">
        <h2 class="icon-pencil-square-o">Editar post #<?= $post->id; ?>
        </h2>
        <a class="icon-link btn btn-green"
    ...
    <div class="dash_content_app_box">
        <form class="app_form"
            action="<?= url("/admin/blog/post/{$post->id}"); ?>"
            method="post">
            <!-- ACTION SPOOFING-->
            ...
            <label class="label">
                ...
                <input type="text" name="title"
                    value="<?= $post->title; ?>"
                    placeholder="A manchete do seu artigo" required />
            </label>
            ...
            <div class="label_g2">
                <label class="label">
                    ...
                    <select name="status" required>
                        <?php
                            ...
                            ?>
                        <option <?= $select("post"); ?>
                            value="post">Publicar</option>
                        <option <?= $select("draft"); ?>
                            value="draft">Rascunho</option>
                        <option <?= $select("trash"); ?>
                            value="trash">Lixo</option>
                    </select>
                </label>
                ...
                <label class="label">
                    <span class="legend">Data de publicação:</span>
                    <input class="mask-datetime" type="text" name="post_at"
                        value="<?= date_fmt($post->post_at, "d/m/Y H:i"); ?>"
                        required />
                </label>
            </div>
